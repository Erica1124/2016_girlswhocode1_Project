...
class Scroller(object):
	...
	def add_buildings(self):
		...
		u=0
		while u < SCREEN_WIDTH:
			width2 = self.add_building(u)
			u += width2
	...
	def add_building(self, x_location):
		...
		global WHITE
		global BLACK
		width2 = random.randint(80,100)
		building = Building(x_location, self.height, width2, random.randint(300,400),self.color)
		building_list1.append(building)
		return width2
		
		
	def draw_buildings(self):
				for buildings in building_list1:
					buildings.draw()
				"""
...
# Scroller(front_scroller)
# Scroller(middle_scroller)
# Scroller(back_scroller)
# -------- Main Program Loop -----------
while not done:
	# --- Main event loop
